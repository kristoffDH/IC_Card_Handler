...
void SHA256_Process( OUT SHA256_INFO *Info, IN const BYTE *pszMessage, IN UINT uDataLen )
{
	...
	while ((uDataLen + remain_buffer) >= SHA256_DIGEST_BLOCKLEN)
	{
		...
		SHA256_Transform((ULONG*)Info->szBuffer, Info->uChainVar);
...
void SHA256_Encrpyt( IN const BYTE *pszMessage, IN UINT uMsgLen, OUT BYTE *pszDigest )
{
	SHA256_INFO info;

	SHA256_Init( &info );

	SHA256_Process( &info, pszMessage, uMsgLen );
